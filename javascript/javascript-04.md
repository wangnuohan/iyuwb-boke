...
| 标志 | 描述             | 备注                                                             |
| ---- | ---------------- | ---------------------------------------------------------------- |
| `g`  | 全局搜索         | 默认情况下只匹配一次，就停止像下匹配了，加上修饰符会一直向下搜索 |
| `i`  | 不区分大小写搜索 | 默认情况下会区分匹配字符串的大小写情况                           |
| `m`  | 多行搜索         | 多行模式，会修改`^`和`$`的行为                                   |
...
| `y` | 执行 粘性搜索匹配从目标字符串的当前位置开始 | - |

### 特殊字符

| 字符 | 含义     | 备注                                                    |
| ---- | -------- | ------------------------------------------------------- |
| `\`  | 转义字符 | 正则表达式中需要匹配特殊字符本身，需要再前面家反斜杠`\` |

正则表达式中，需要反斜杠转义的：
`^`,`.`,`[`,`$`,`(`,`)`,`|`,`*`,`+`,`?`,`{`,`\` |
| `^` | 匹配开始位置 | 如果设置多行标志，那么也匹配换行符后的位置
例如：
`/^A/`会匹配 `"Ant"`中的`A`，但是不会匹配`"ntA"`中的`A` |
| `$` | 匹配结束位置 | 如果设置多行标志，那么也匹配换行符前的位置
例如：
`/A$/`会匹配 `"ntA"`中的`A`，但是不会匹配`"Ant"`中的`A` |
| `*` | 匹配一个表达式 0 次或多次 | 等价于 `{0, }`
例如：
`/yueno*/`会匹配 `"yuenoooyuen"`中的`yuenooo`和`yuen` |
| `+` | 匹配一个表达式 1 次或多次 | 等价于 `{1, }`
例如：
`/yueno+/`只会匹配 `"yuenoooyuen"`中的`yuenooo` |
| `?` | 匹配一个表达式 0 次或 1 次 | 等价于`{0,1}`
例如：
`/yueno?/`只会匹配 `"yuenoooyuen"`中的`yueno`
注意：
`?`如果紧跟再任何量词 `*`、`+`、`?`或`{}`的后面，将会使量词变得非贪婪（匹配尽量少的字符）
例如：
`/yueno??/`只会匹配 `"yuenoooyuen"`中的`yuen` |
| `.` | 默认匹配出换行符之外的任何单个字符 | 例如：
`/.y/`只会匹配 `"yuenoooyuen"`中的`oy`
`/..y/`只会匹配 `"yuenoooyuen"`中的`ooy` |
| `(x)` | 捕获括号 | 正则表达式中的括号表示分组匹配，括号中的模式可以用匹配分组的内容
分组匹配可以使用`\n`
例如：
`/(wenbo)+/.test('wenbowenbo')` 为`true`，表示匹配`wenbo`整体一次或多次
正则替换中，可以使用`$1`,`$2`语法
例如：
`"wenbo,zhijian".replace(/(wenbo),(zhijian)/, '$2,$1')`

输出为：`zhijian,wenbo` |
| `(?:x)` | 匹配`X`但是不记住匹配项 | 非捕获括号，使你能够定义与正则表达式运算符一起使用的子表达式
使用非捕获括号，能匹配元素，但是不能在使用`\n`和`$n`方法 |
| `x(?=y)` | 匹配`x`，仅仅当`x`后面是`y` | 先行断言
例如：
`'wenbo'.match(/wen(?=bo)/)`
输出：`[ 'wen', index: 0, input: 'wenbo', groups: undefined ]`
`'wenyu'.match(/wen(?=bo)/)`
输出：null |
| `(?<=y)x` | 匹配`x`，仅仅当`x`前面是`y` | 后行断言
例如：
`'wenbo'.match(/(?<=wen)bo/)`
输出：`[ 'bo', index: 3, input: 'wenbo', groups: undefined ]`
`'yubo'.match(/(?<=wen)bo/)`
输出：null |
| `x(?!y)` | 匹配`x`，仅仅当`x`后面不是`y`时 | 正向否定查找
|
| `(?<=y)x` | 匹配`x`，仅仅当`x`前面不是`y` | 反向否定查找
|
| `x|y` | 匹配`x`或者`y` | 或者，可以连用
例如：
`'wenyu'.match(/w|e|n/g)`
输出：`[ 'w', 'e', 'n' ]` |
| `{n}` | 匹配前面一个字符刚好出现`n`次，`n`是一个正整数 |
例如：
`'hello'.match(/l{2}/g)`
输出：`[ 'll' ]` |
| `{n,}` | 匹配一个字符至少出现了`n`次，`n`是一个正整数 | - |
| `{n,m}` | 匹配前面的字符至少`n`次最多`m`次，`n`，`m`是为正整数 | - |
| `[xyz]` | 字符集合
| 匹配方括号中的任意字符，包括转义字符，可以使用破折号(-)制定一个字符范围,例如：`[a-zA-Z1-9]`
例如：
`'hello 123'.match(/[a-h1-2]/g)`
输出：
`[ 'h', 'e', '1', '2' ]` |
| `[^xyz]` | 反向字符集 | 匹配任何没有包含再方括号中的字符
例如：
`'hello 123'.match(/[^a-h1-2]/g)`
输出：` [ 'l', 'l', 'o', '3' ]` |
| `[\b]` | 匹配一个退格(U+0008) | 不是`\b`，不要弄混了 |
| `\b` | 匹配一个词的边界 | `/\bworld/.test('hello world') // true`
`/\bworld/.test('hello-world') // true`
`/\bworld/.test('helloworld') // false` |
| `\B` | 匹配一个非单词边界 | `/\bworld/.test('hello world') // false`
`/\bworld/.test('hello-world') // false`
`/\bworld/.test('helloworld') // true` |
| `\cX` | 当 X 是出于 A 到 Z 之间字符的时候，匹配字符串中的一个控制符 | - |
| `\d` | 匹配一个数字 | 等价于`[0-9]` |
| `\D` | 匹配一个非数字字符 | 等价于`[^0-9]` |
| `\f` | 匹配一个换页符(U+000C) | - |
| `\n` | 匹配一个换行符(U+000A) | - |
...
| `\W`
| 匹配一个非单字字符 | - |
| `\n` | 返回最后的第 n 个字捕获匹配的子字符串 | 捕获的数目以左括号计算 |
| `\0` | 匹配 NULL 字符（U+0000） | - |
| `\xhh` | 匹配一个两位十六进制数表示的字符（\x00-\xFF） | - |
| `\uhhhh` | 匹配一个四位十六进制数表示的 UTF-16 代码单元 | - |
| `\u{hhhh}`
或
`\u{hhhhh}` | （仅当设置了 U 标志时）匹配一个十六进制时候表示的 Unicode 字符 | - |
